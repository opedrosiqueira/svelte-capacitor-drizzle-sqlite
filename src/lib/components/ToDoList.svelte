<script>
  let { notas, notaEditando, conteudoNotaEditando = $bindable(), confirmarEdicao, cancelarEdicao, alterarStatus, editarNota, excluirNota, status = 1 } = $props();
...
{#each notas as nota}
  <div class="input-group mb-1 {textDecoration}">
    ...
    {:else}
      <button class="btn btn-success input-group-text" aria-label="concluir" onclick={() => alterarStatus(nota, status)}><i class="bi bi-{statusButton}"></i></button>
